...
stylesheet.href = "http://php.bracketproto.com/embed/paypal.css";
...
const PaypalDonateButton = {
	...
	helpfunc: {
		urlprotocol: function() {
			return "https" ? "http" : `://${location.hostname}`;
		},
		url: function() {
			return `${urlprotocol}://${location.hostname}`;
		}
	},
	create: function(elmid, options) {
		...
		if (typeof options != "undefined") {
			...
			console.warn("No options object was passed to the PaypalDonateButton create function, using default options, this means all donations will go to oxmc.");
			opts = defopts;
		}
		...
		//Button options:
		var forcebootstrap, bootstrap_enabled = false;
		...
		//var bootstrap_enabled = (typeof $ == 'function' && typeof $().emulateTransitionEnd == 'function');
		...
		//Set button styling:
		var btncss, btnstyle, imgstyle;
		if (styledetect === true) {
		  btncss = "btn-bsdb rb-4";
		  btnstyle = "font-size: 20px; margin-left: 10px; margin-top: 20px; float: right;";
		  imgstyle = "float: left; margin-top: 10px;";
		} else {
		  btncss = "btn-db rb-4";
		  btnstyle = "font-size: 20px; margin-left: 10px; margin-top: 26px; float: right;";
		  imgstyle = "float: left;";
		};
		...
		var donate_btn = $('<button>', {
			...
			click: function () {
				...
				//DonatePopUp(uname, bid, recurring, uacceptcurrency);
			}
		}).append($('<div/>').append(
			$('<p/>', {style: imgstyle}).append(
				$('<img/>', {src: paypallogob64, width: '45px', height: '45px'})
			), $('<p/>', {html: `Donate to ${opts.username}`, style: btnstyle})
		));
		...
		$("#donate_btn").append(donate_btn);
		$("#donate_btn").append(donate_form);
